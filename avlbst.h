...
protected:
    ...
    void insertHelper(AVLNode<Key, Value>* parent, AVLNode<Key, Value>* current);
    void removeHelper(AVLNode<Key, Value>* current, int diff);
...
void AVLTree<Key, Value>::insert (const std::pair<const Key, Value> &new_item)
{
   ...
   if(parent->getBalance() == -1 || parent->getBalance() == 1)
    ...
    else if(parent->getBalance() == 0)
    {
      ...
      insertHelper(parent, current);     
    }
}

template<class Key, class Value>
void AVLTree<Key, Value>::insertHelper(AVLNode<Key, Value>* parent, AVLNode<Key, Value>* current)
{
...
void AVLTree<Key, Value>:: remove(const Key& key)
{
    ...
    delete current;
    removeHelper(currParent, diff);
}

template<class Key, class Value>
void AVLTree<Key, Value>::removeHelper(AVLNode<Key, Value>* current, int diff)
{
  if(current == nullptr)
  {
    return;
  ...
  if(diff == -1)
  {
    ...
    {
      AVLNode<Key, Value>* tallerChild=current->getLeft();
      if(tallerChild->getBalance() == -1)
      {
        rotateRight(current);
        current->setBalance(0);
        tallerChild->setBalance(0);
        removeHelper(parent, diff_);
      }
  ...
  else if(diff == 1)
  {
    if(current->getBalance() + diff ==2)
    {
      AVLNode<Key, Value>* tallerChild=current->getRight();
      if(tallerChild->getBalance() == 1)
      {
        rotateLeft(current);
        current->setBalance(0);
